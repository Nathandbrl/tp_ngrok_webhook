...
# Ensure absolute path for deployment directory if it's relative
if (-not (Test-Path $deployDir -PathType Container)) {
    # If directory doesn't exist, we might be cloning.
    # Keep path as is, git clone will handle it or we create it.
    # But if it starts with .., resolve it relative to current location
    $deployDir = Resolve-Path -Path $deployDir -ErrorAction SilentlyContinue
    if (-not $deployDir) {
        # Path doesn't exist yet, construct full path if possible or leave as is
        $deployDir = $env:DEPLOY_DIR
    }
}
